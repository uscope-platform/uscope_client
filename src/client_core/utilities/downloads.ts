...
export let download_plot = (channels: any, file_name: string) => {
    let timebase = channels[0].x;
    let data = channels.map((ch : any)=>{
        return ch.y;
    });
    let csv_content;

    csv_content = `timebase,${channels[0].name},${channels[1].name},${channels[2].name},${channels[3].name},${channels[4].name},${channels[5].name}\n`
    for(let i = 0; i<data[0].length; i++){

        csv_content += `${timebase[i]},${data[0][i]},${data[1][i]},${data[2][i]},${data[3][i]},${data[4][i]},${data[5][i]}\n`
    }

...
class App extends Component {
    ...
    constructor(props) {
        ...
        let server = new serverProxy('http://0.0.0.0:8989/uscope/', ''); // unimore:http://155.185.48.185/uscope/ docker:http://172.18.0.1:4999/uscope/ unnc:http://10.190.0.74:4999/uscope/
        ...
        this.state.server.auth_proxy.sign_in(login_credentials).then((token) =>{
            debugger;
            let server = new serverProxy('http://0.0.0.0:8989/uscope/',token.access_token); // unimore:http://155.185.48.185/uscope/ docker:http://172.18.0.1:4999/uscope/ unnc:http://10.190.0.74:4999/uscope/
            if(token.login_type === 'automated') localStorage.setItem('login_token', JSON.stringify(token.login_token));

...
import {InputField, SimpleContent, UIPanel, RangedInputField, Checkbox} from "#UI/index.js";
import {up_emulator} from "#client_core/index.js";


const ranges_map = {s:1, ms:1e-3, us:1e-6} as const;

let get_range = (time: number):[string, number] =>{
    if(time !== 0){
        let new_range;
...
interface EmulatorPropertiesProps {
    selected_emulator: up_emulator,
    enabled: boolean,
}

let  EmulatorProperties = (props: EmulatorPropertiesProps) =>{
    ...
    let handle_change = async (event: React.KeyboardEvent<HTMLInputElement>) =>{
        if(event.key==="Enter"|| event.key ==="Tab") {
            if (event.currentTarget.name === "name"){
                await props.selected_emulator.edit_name(event.currentTarget.value);
                forceUpdate();
            } else if(event.currentTarget.name === "emulation_time"){
                set_indicated_time(parseFloat(event.currentTarget.value));
                let time = parseFloat(event.currentTarget.value)*ranges_map[range as keyof typeof ranges_map];
    ...
    const handle_range_change = async (value: string)=>{
        let time = indicated_time*ranges_map[value as keyof typeof ranges_map];
        await props.selected_emulator.edit_emulation_time(time);
        forceUpdate();
    }

    const handle_deployment_mode = async (event: React.ChangeEvent<HTMLInputElement>) =>{

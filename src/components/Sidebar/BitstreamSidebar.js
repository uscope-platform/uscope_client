...
import {Button, FormLayout, InputField, SimpleContent, UIPanel} from "../UI_elements";
...
let  BitstreamSidebar = props =>{
    ...
    let upload_file = (event) => {
        up_bitstream.get_file_content( inputFile).then((file_content =>{
            selected_bitstream.edit_field("file_content", file_content.content)
        }));
    }

    let handle_change_name = (event) =>{
        if(event.key==="Enter"|| event.key ==="Tab"){
            selected_bitstream.edit_field(event.target.name, event.target.value);
        }
    }
